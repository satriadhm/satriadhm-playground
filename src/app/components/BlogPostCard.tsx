// src/app/components/BlogPostCard.tsx - Updated with pure Tailwind dark mode
'use client';
...
export default function BlogPostCard({ post, onClick, variant = 'list' }: BlogPostCardProps) {
  ...
  if (variant === 'featured') {
    return (
      <article 
        className="border-b border-slate-200 dark:border-slate-700 pb-16 sm:pb-20 cursor-pointer group transition-all duration-300"
        onClick={onClick}
      >
        <div className="space-y-6 sm:space-y-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-sm text-slate-600 dark:text-slate-400">
              ...
              <span className="px-3 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded-full text-xs font-medium border border-blue-200 dark:border-blue-800">
                {post.category}
              </span>
              <span className="hidden sm:inline text-slate-400 dark:text-slate-500">·</span>
              <div className="flex items-center space-x-1">
      ...
      >
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <span className="px-3 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded-lg text-sm font-medium border border-blue-200 dark:border-blue-800">
              {post.category}
            </span>
            {post.featured && (
              <span className="px-2 py-1 bg-yellow-50 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400 rounded text-xs font-medium border border-yellow-200 dark:border-yellow-800">
                Featured
              </span>
            )}
          </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start gap-4 sm:gap-8 p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-lg transition-colors">
        ...
        <div className="flex-grow space-y-3 order-1 sm:order-2">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-sm text-slate-600 dark:text-slate-400">
            <span className="px-2 sm:px-3 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded-full text-xs border border-blue-200 dark:border-blue-800">
              {post.category}
            </span>
            <div className="flex items-center space-x-1">
