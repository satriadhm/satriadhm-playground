...
export default function ExperienceSection() {
  const [selectedType, setSelectedType] = useState<
    "all" | "fulltime" | "parttime" | "internship"
  >("all");
  const [expandedExperience, setExpandedExperience] = useState<string | null>(
    null
  );
  const [selectedExperienceModal, setSelectedExperienceModal] =
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12 sm:mb-16">
          ...
          <p className="text-base sm:text-lg text-slate-600 dark:text-slate-400 max-w-3xl mx-auto px-4">
            My journey through various roles in software engineering, from
            internships to full-time positions, building scalable systems and
            leading innovative projects.
          </p>
        </div>

        <div className="flex justify-center mb-8 sm:mb-12 px-4">
          {/* 1️⃣  Hilangkan max-width */}
          <div className="w-full">
            {/* 2️⃣  Wajib full-width  +  distribusi ruang */}
            <div
              className="hidden xs:flex w-full justify-between
                    bg-white dark:bg-slate-800 rounded-lg p-1 shadow-sm
                    border border-slate-200 dark:border-slate-700"
            >
              {(["all", "fulltime", "parttime", "internship"] as const).map(
                (type) => (
                  <button
                    key={type}
                    onClick={() => setSelectedType(type)}
                    /* 3️⃣  Biarkan tombol “stretch” */
                    className={`flex-1 flex items-center justify-center space-x-2
                      px-3 sm:px-6 py-2 sm:py-3 rounded-md
                  ...
                  >
                    ...
                    {type !== "all" && (
                      <span
                        className="text-xs bg-slate-200 dark:bg-slate-600
                  ...
                  <div className="flex-grow">
                    <div className="flex flex-col space-y-3 sm:space-y-4 mb-4 sm:mb-6">
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4">
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-3">
                          <span
                            className={`flex items-center space-x-2 px-3 py-2 rounded-lg text-sm font-medium border ${getTypeColor(
                              experience.type
                            )}`}
                          >
                          ...
                          {(experience.achievements?.length ||
                            imageExists[experience.id] ||
                            experience.certificates?.length) && (
                            <button
                              onClick={() =>
                                setSelectedExperienceModal(experience)
                              }
                    ...
                    <div className="space-y-3 sm:space-y-4 mb-4 sm:mb-6">
                      {experience.description
                        .slice(
                          0,
                          expandedExperience === experience.id ? undefined : 2
                        )
                        .map((desc, descIndex) => (
                          <p
                            key={descIndex}
                            ...
                            <p className="text-xs sm:text-sm text-slate-700 dark:text-slate-300 font-medium">
                              ...
                              {experience.achievements.length > 1 && (
                                <span className="text-slate-500 dark:text-slate-400">
                                  {" "}
                                  and {experience.achievements.length - 1}{" "}
                                  more...
                                </span>
                              )}
                            </p>
            ...
            <div className="p-4 sm:p-6 space-y-6 sm:space-y-8">
              ...
              <div>
                ...
                <div className="space-y-3 sm:space-y-4">
                  {selectedExperienceModal.description.map(
                    (desc, descIndex) => (
                      <p
                        key={descIndex}
                        className="text-sm sm:text-base text-slate-700 dark:text-slate-300 leading-relaxed"
              ...
              {/* All Achievements */}
              {selectedExperienceModal.achievements &&
                selectedExperienceModal.achievements.length > 0 && (
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      {selectedExperienceModal.achievements.map(
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 sm:gap-3">
                  {selectedExperienceModal.technologies.map(
                    (tech, techIndex) => (
                      <span
                        key={techIndex}
              ...
              {imageExists[selectedExperienceModal.id] &&
                selectedExperienceModal.images &&
                selectedExperienceModal.images.length > 0 && (
                  <div>
                    <h4 className="text-base sm:text-lg font-semibold text-slate-900 dark:text-slate-100 mb-3 sm:mb-4">
                      Project Gallery
                    </h4>
              ...
              {selectedExperienceModal.certificates &&
                selectedExperienceModal.certificates.length > 0 && (
                  <div>
                    <h4 className="text-base sm:text-lg font-semibold text-slate-900 dark:text-slate-100 mb-3 sm:mb-4">
                      Certificates & Recognition
                    </h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      {selectedExperienceModal.certificates.map(
                        (cert, certIndex) => (
                          <a
                            key={certIndex}
                            href={cert}
                          ...
                          >
                            <ExternalLink
                              size={18}
                              className="text-blue-500 group-hover:text-blue-700 dark:group-hover:text-blue-300 flex-shrink-0"
                            />
